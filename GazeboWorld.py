...
class GazeboWorld():
	def __init__(self):
		...
		self.action_table = [[0.5, np.pi/12],[0.5, 0],[0.5, -np.pi/12],[0.2, np.pi/6],[0.2, -np.pi/6],[0.2, np.pi/12],[0.2, -np.pi/12],[0.2, 0],[0, np.pi/6],[0, -np.pi/6]]#[0.4, 0.2, np.pi/6, np.pi/12, 0., -np.pi/12, -np.pi/6]
	...
	def GetDepthImageObservation1(self):
		# ros image to cv2 image
	...
	def SetObjectPoseTest(self, name='mobile_base', index = random.randint(0,4)):
		pos = [[2.5,-2.0,0.], [2.5,-2.0,2*np.pi/5], [2.5,-2.0, 4*np.pi/5], [2.5,-2.0, 6*np.pi/5], [2.5,-2.0, 8*np.pi/5]]
		#pos = [[2.,-4.,0.], [2.,-4.,2*np.pi/5], [2.,-4., 4*np.pi/5], [2.,-4., 6*np.pi/5], [2.,-4., 8*np.pi/5]]
		#quaternion = tf.transformations.quaternion_from_euler(0., 0., np.random.uniform(-np.pi, np.pi))
		#num = random.randint(0,4)
		num = index
		#print(pos[num])
		quaternion = tf.transformations.quaternion_from_euler(0.,0.,pos[num][2])
		if name is 'mobile_base' :
			object_state = copy.deepcopy(self.set_self_state)
			object_state.pose.position.x = pos[num][0]
			object_state.pose.position.y = pos[num][1]
			object_state.pose.orientation.x = quaternion[0]
			object_state.pose.orientation.y = quaternion[1]
			object_state.pose.orientation.z = quaternion[2]
			object_state.pose.orientation.w = quaternion[3]
	...
	def Getegomotion(self):
		[v, w] = self.GetSelfOdomeSpeed()
		return [v, w]
	def ResetWorld(self):
	...
	def ResetWorldTest(self, times):
		self.SetObjectPoseTest(index = times) # reset robot
		for x in xrange(len(self.object_name)): 
			self.SetObjectPose(self.object_name[x]) # reset target
		self.self_speed = [.4, 0.0]
		self.step_target = [0., 0.]
	...
	def Control(self, action):
		...
		self.self_speed[0] = self.action_table[action][0]
		self.self_speed[1] = self.action_table[action][1]
		move_cmd = Twist()
	...
	def GetRewardAndTerminate(self, t, w_t1):
		terminate = False
		reset = False
		...
		reward_w = 0
		reward_c = 0
		if self.GetBump() or np.amin(laser) < 0.035:
			reward = -10.
			terminate = True
			reset = True
			return reward, terminate, reset	
		if (np.amin(laser) < 0.1):
			reward_c = 0.05*(0.1/np.amin(laser))
		if (theta*w_t1 < 0):
			reward_w = 0.05*abs(theta - w_t1)
		#print(theta*w_t1)
		reward = v * v * np.cos(v * theta) * 2 - reward_w - reward_c
